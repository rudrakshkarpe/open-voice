...
The playback hook owns play intent separately from actual playback. It pauses video when a required translated section is pending, prefetches one section ahead, and applies source-to-generated duration mapping to the audio clock. Pause, seek, source replacement, and language changes invalidate pending play operations. The caption renderer splits each section into short, Unicode-aware phrases and estimates their boundaries proportionally to text length. It selects a single phrase from the video clock (not a typing animation), hides captions at zero, and never displays incoming translation deltas as playback captions. These are not provider word timestamps. Further latency work should measure time to the first transcript, translated text, first speech byte, and resumed playback separately.
