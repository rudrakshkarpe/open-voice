...
5. Selecting a language and pressing play translates the current and next section from the verified source text. Translation text arrives progressively; Higgs TTS generates a cached MP3 for each section.
6. Playback waits when a section is unavailable. Captions follow the source timeline; translated audio is time-stretched to the section, with video slowed for extreme duration differences. Pause and seek affect both media clocks. Language changes stop the previous voice immediately.
